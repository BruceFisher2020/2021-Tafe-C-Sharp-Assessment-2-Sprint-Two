...
namespace AstronomicalProcessing
{
    public partial class AstronomicalProcessingForm : Form
    {
        ...
        static int InputLowestRange = 10; // lowest value input range
        ...
        private void ButtonMode_Click(object sender, EventArgs e)
        {
            ...
            int[] Modes = new int[ArraySize]; // stores all Modes for current Valid Data size
            int[] Frequencies = new int[ArraySize]; // stores all Frequencies for current Valid Data size
            ...
            if (numberOfDataEntries != 0)
            {
                for (int i = 0; i < NeutrinoInteractions.Length; i++)
                {
                    if (NeutrinoInteractions[i] != 0)
                    {
                        ...
                        if (counter >= highestFrequency)
                        {
                            ...
                            Frequencies[i] = counter; // store multiple frequencies
                            Modes[i] = element; // store multiple modes
                        }
                    }
                }
                ...
                {
                    for (int j = 0; j + 1 < Modes.Length; j++)
                    {
                        temp = Modes[j];
                        temp2 = Frequencies[j];
                ...
                for (int i = 0; i < Modes.Length; i++)
                {
                    if (Frequencies[i] != 1 && Frequencies[i] == highestFrequency && Modes[i] != previousListedMode)
                    {
                        modesListed = true;
                        previousListedMode = Modes[i];
                        ListBoxMode.Items.Add(String.Format("{0,3} {1,12}", Modes[i], Frequencies[i]));
                    }
                }
        ...
        private void TextBoxInput_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                // trap user attempt to enter first number as 0, and check if InputLowestRange is larger than 9
        ...
        /// </summary>
        /// <param name="sortArray">Array to be sorted</param>
        private void SortIntArray(int[] sortArray)
        {
            int temp;
            for (int i = 0; i < sortArray.Length; i++)
            {
                for (int j = 0; j + 1 < sortArray.Length; j++)
                {
                    temp = sortArray[j];
